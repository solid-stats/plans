...
the user.** New SG corpus reconciles 1:1 with the live legacy corpus — every per-rotation delta
named and accepted. **F9/F5 are now DEPLOYED to staging** (image `0fe3326`) on a right-sized 1Gi pod
with `NODE_OPTIONS=--max-old-space-size=768` (clean rollout, 0 restarts) — the OOM on the first
attempt was the pod under-provisioning, since resolved. F14 → v4. See the **Update 2026-06-17** note
...
>
> **Update 2026-06-17 — coverage closed + SG value parity re-measured + F9/F5 deployed (authoritative; supersedes the dated figures above).**
>
...
> `includeReplays`-by-name match — new is the correct side) **− 2 source-deleted-on-sg.zone**
> (`1757773065`, `1773140513`, both now HTTP 404, recoverable only from the legacy cache). R00pre
> (the 9 earliest sg replays) is the identical set on both sides. Corpus 23556 → **23682**.
>
> *(b) SG VALUE parity re-measured (new vs legacy snapshot).* **+0.5% games / kills / deaths,
> +0.9% players — new is now slightly HIGHER** (was −1.5% / −1.1% / −1.4% players on 2026-06-15).
> **deaths went −13% → +0.5%**, fixed via **F12** (`replay_timestamp` backfill) + **F13a** (one-life
> deaths cap, ≤1/game) + **deaths-counter-from-`raw_snapshot`**. **ZERO non-rename player differs by
> ≥40 kills** → no computation divergence remains. The big per-player swings decompose to the **F14
...
> server-2 image `0fe3326`; the server-2 pod was raised **512Mi → 1Gi** with
> `NODE_OPTIONS=--max-old-space-size=768` after the F5-burst OOM. Clean rollout, 0 restarts.
>
> *(d) F14 (identity / `nameChanges` port) DEFERRED to v4* — becomes the name-change domain entity +
